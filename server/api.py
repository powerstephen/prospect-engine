...
async def generate_report_url(contact_id: int):
    import re as _re, os as _os, httpx as _httpx
    supabase_key = _os.environ.get("SUPABASE_SERVICE_KEY", "")
    supabase_url = "https://neonmrgszujadgfidlbj.supabase.co"
    headers = {"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}", "Content-Type": "application/json"}
    async with _httpx.AsyncClient(timeout=10) as c:
        r = await c.get(f"{supabase_url}/rest/v1/contacts?id=eq.{contact_id}&select=company,location", headers=headers)
        data = r.json()
    if not data:
    ...
    async with _httpx.AsyncClient(timeout=10) as c:
        await c.patch(
            f"{supabase_url}/rest/v1/contacts?id=eq.{contact_id}",
            json={"report_slug": slug, "report_url": report_url},
